...
HF_PATH = 'iluvvatar/NEREL'


@dataclass(repr=False)
class NEREL(HFBasedDataset):
    """
    https://arxiv.org/abs/2108.13112
    """
    ...
    @classmethod
    def load_from_disk(cls, path: PathLike) -> 'NEREL':
        path = Path(path)
    ...
    @property
    def rel_types(self) -> List[Tuple[str, List[str], List[str]]]:
        return [(row['type'], row['arg1'], row['arg2'])
                for row in self._rel_types]
